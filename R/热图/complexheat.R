...
pacman::p_load("lmbio","ComplexHeatmap")
outname <- ""
group <- readdata("data.xlsx",1)
...
heatdata <- merge(group,data[c('Accession',consamp,casesamp,"FC")],by="Accession")
name <- c("FAM","Other")
name <- c("CM","LAM","BAM")
name <- c("HI","HF","HG")
...
ht_list <- Heatmap(heatdf,
  ...
  anno_right
pdf(paste0(outname,".pdf"), width = 3.81, height =6.68)
#grid.newpage()
...
savexlsx(list("输入数据"=heatdata1[c("Accession","蛋白:基因","Pathway",c(consamp,casesamp),"FC")],
